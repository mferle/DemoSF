...
-- varianta (4)
-- isto kot varianta (1), samo da prepišemo poizvedbo z with
-- v pogoju imamo cross join s poizvedbo iz with

--select * from adm.adm_inc_load;
--update adm.adm_inc_load set is_loaded = 1 where load_date = '2020-07-28';

create or replace view TV_MET_ZAVAROVALNI_PROD_A_26 as 
with ld_dt as (select min(load_date) as load_date from adm.ADM_INC_LOAD where is_loaded = 0)
...
select * from ld_26_plc_cmp_42_dvojcki;
